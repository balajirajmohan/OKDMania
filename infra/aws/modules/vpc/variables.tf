...
variable "private_subnet_cidrs" {
  description = <<-EOT
    CIDR blocks for private subnets. Leave [] to skip private subnets
    entirely. When set, the module creates private subnets + a private route
    table.
  EOT
  type        = list(string)
  default     = []
...
variable "enable_nat_gateway" {
  description = <<-EOT
    ...
    Costs ~$32/month + data. Only meaningful when private_subnet_cidrs is
    non-empty.
  EOT
  type        = bool
  default     = false
